...
bool PaletteReader::Load(const std::string& f)
{
	...
	for (int i = 0; i < 256; i++)
	{
		...
		g_palette.pal[i] = { gammatable[r], gammatable[g], gammatable[b], 255 };
	}
	Log::Print("Palette loaded.\n");
	return true;
}

void PaletteReader::LoadDefault()
{
	byte lmppal[768] = {
...
void PaletteReader::GenerateErrorPalette()
{
	Log::Print("Generating hideous error-colored palette ...\n");

	for (int i = 0; i < 256; i++)
	{
		byte x = (byte)(i % 255);
		g_palette.pal[i] = { x, 0, x, 255 };
	}
}

void PaletteReader::GenerateGammaTable(byte* gammatable)
{
	int		inf;
	float	v;
	...
	for (int i = 0; i < 256; i++)
	{
		...
		v = (float)(255.0f * pow((v + 0.5) / 255.5, (float)g_cfgUI.Gamma) + 0.5f);
		inf = (int)ceil(max(0.0f, min(255.0f, v)));
		gammatable[i] = inf;
	}
}