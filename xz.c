...
const char *g_pszDimStringsXZ[] = {"x:%.f", "y:%.f", "z:%.f"};
const char *g_pszOrgStringsXZ[] = {"(x:%.f  y:%.f)", "(x:%.f  z:%.f)", "(y:%.f  z:%.f)"};
...
void XZ_Init ()
{
	g_qeglobals.d_xz.origin[0] = 0;
	g_qeglobals.d_xz.origin[1] = 0;	// sikk - changed from "20"
	g_qeglobals.d_xz.origin[2] = 0;	// sikk - changed from "46"
	g_qeglobals.d_xz.scale = 1;
...
void XZ_PositionView ()
{
	...
	if (b && b->next != b)
	{
		g_qeglobals.d_xz.origin[nDim1] = b->mins[nDim1];
		g_qeglobals.d_xz.origin[nDim2] = b->mins[nDim2];
	}
	else
	{
		g_qeglobals.d_xz.origin[nDim1] = g_qeglobals.d_camera.origin[nDim1];
		g_qeglobals.d_xz.origin[nDim2] = g_qeglobals.d_camera.origin[nDim2];
	}
}
...
{
	point[0] = g_qeglobals.d_xz.origin[0] + (x - g_qeglobals.d_xz.width / 2) / g_qeglobals.d_xz.scale;
	point[2] = g_qeglobals.d_xz.origin[2] + (y - g_qeglobals.d_xz.height / 2) / g_qeglobals.d_xz.scale;
}
...
void XZ_SnapToPoint (int x, int y, vec3_t point)
{
	if (g_qeglobals.d_savedinfo.bNoClamp)
		XZ_ToPoint(x, y, point);
	else
		XZ_ToGridPoint(x, y, point);
}
...
bool XZ_Drag_Delta (int x, int y, vec3_t move)
{
	...
	xvec[0] = 1 / g_qeglobals.d_xz.scale;
	xvec[1] = xvec[2] = 0;
	yvec[1] = 1 / g_qeglobals.d_xz.scale;
	yvec[0] = yvec[2] = 0;
...
void XZ_MouseDown (int x, int y, int buttons)
{
	...
	else
	{
		...
		right[0] = 1 / g_qeglobals.d_xz.scale;
		right[1] = 0;
		right[2] = 0; 

		up[0] = 0; 
		up[1] = 0;
		up[2] = 1 / g_qeglobals.d_xz.scale;
...
//			g_qeglobals.d_camera.origin[1] = point[1];
			XZ_VectorCopy(point, g_qeglobals.d_camera.origin);
...
// sikk---> Free Rotate		
			...
			if (GetKeyState(VK_MENU) < 0)
			{
				...
				sprintf(szRotate, "Rotate: %d°", g_nRotateXZ);
...
// <---sikk
			else
			{
				if (x != cursorx || y != cursory)
				{
					...
					g_qeglobals.d_xz.origin[nDim1] -= (x - cursorx) / g_qeglobals.d_xz.scale;
					g_qeglobals.d_xz.origin[nDim2] += (y - cursory) / g_qeglobals.d_xz.scale;

					Sys_SetCursorPos(cursorx, cursory);
					Sys_UpdateWindows(W_XY| W_Z);

					sprintf(xystring, "xyz Origin: (%d %d %d)", (int)g_qeglobals.d_xz.origin[0], (int)g_qeglobals.d_xz.origin[1], (int)g_qeglobals.d_xz.origin[2]);
					Sys_Status(xystring, 0);
				}
			}
			return;
...
// sikk---> Mouse Zoom
		...
		if (buttonstate == (MK_CONTROL | MK_RBUTTON))
		{
			...
			if (y != cursory)
			{
				...
				g_qeglobals.d_xz.scale *= scale;

				if (g_qeglobals.d_xz.scale > 32)
					g_qeglobals.d_xz.scale = 32;
				else if (g_qeglobals.d_xz.scale < 0.05)
					g_qeglobals.d_xz.scale = (float)0.05;
...
void XZ_DrawGrid ()
{
	...
	w = g_qeglobals.d_xz.width / 2 / g_qeglobals.d_xz.scale;
	h = g_qeglobals.d_xz.height / 2 / g_qeglobals.d_xz.scale;

	nDim1 = 0;
	nDim2 = 2;

	xb = g_qeglobals.d_xz.origin[nDim1] - w;
	if (xb < g_v3RegionMins[nDim1])
		xb = g_v3RegionMins[nDim1];
	xb = nSize * floor(xb / nSize);

	xe = g_qeglobals.d_xz.origin[nDim1] + w;
	if (xe > g_v3RegionMaxs[nDim1])
		xe = g_v3RegionMaxs[nDim1];
	xe = nSize * ceil(xe / nSize);

	yb = g_qeglobals.d_xz.origin[nDim2] - h;
	if (yb < g_v3RegionMins[nDim2])
		yb = g_v3RegionMins[nDim2];
	yb = nSize * floor(yb / nSize);

	ye = g_qeglobals.d_xz.origin[nDim2] + h;
	...
	// draw minor blocks
	if (g_qeglobals.d_bShowGrid && g_qeglobals.d_nGridSize * g_qeglobals.d_xz.scale >= 4)
	{
...
// sikk---> Filter Coords so they don't become bunched and unreadable 
	...
	if (g_qeglobals.d_savedinfo.bShow_Coordinates)
	{
		...
		nSize = 64;
		if (g_qeglobals.d_xz.scale <= 0.6)
			nSize = 128;
		if (g_qeglobals.d_xz.scale <= 0.3)
			nSize = 256;
		if (g_qeglobals.d_xz.scale <= 0.15)
			nSize = 512;
		if (g_qeglobals.d_xz.scale <= 0.075)
			nSize = 1024;
		...
		{
			glRasterPos2f(x, g_qeglobals.d_xz.origin[nDim2] + h - 6 / g_qeglobals.d_xz.scale);
			sprintf(text, "%d",(int)x);
			glCallLists(strlen(text), GL_UNSIGNED_BYTE, text);
		}
		for (y = yb; y <= ye; y += nSize)	// sikk - 'y <= ye' instead of 'y < ye' so last coord is drawn
		{
			glRasterPos2f(g_qeglobals.d_xz.origin[nDim1] - w + 1, y);
...
// sikk---> View Axis - cleaner, more intuitive look
	if (g_qeglobals.d_savedinfo.bShow_Viewname)
	{
		...
		glRasterPos2f(g_qeglobals.d_xz.origin[nDim1] - w + 68 / g_qeglobals.d_xz.scale, 
					  g_qeglobals.d_xz.origin[nDim2] + h - 51 / g_qeglobals.d_xz.scale);
		glCallLists(1, GL_UNSIGNED_BYTE, &szView[0]);

		glColor3fv(g_qeglobals.d_savedinfo.v3Colors[COLOR_VIEWNAME]);
		glRasterPos2f(g_qeglobals.d_xz.origin[nDim1] - w + 44 / g_qeglobals.d_xz.scale, 
				      g_qeglobals.d_xz.origin[nDim2] + h - 28 / g_qeglobals.d_xz.scale);
		glCallLists(1, GL_UNSIGNED_BYTE, &szView[1]);

		glLineWidth(2);
		glColor3fv(p1);
		glBegin(GL_LINES);
		glVertex2f(g_qeglobals.d_xz.origin[nDim1] - w + 48 / g_qeglobals.d_xz.scale, 
				   g_qeglobals.d_xz.origin[nDim2] + h - 48 / g_qeglobals.d_xz.scale);
		glVertex2f(g_qeglobals.d_xz.origin[nDim1] - w + 64 / g_qeglobals.d_xz.scale, 
				   g_qeglobals.d_xz.origin[nDim2] + h - 48 / g_qeglobals.d_xz.scale);
		glEnd();
		...
		glVertex2f(g_qeglobals.d_xz.origin[nDim1] - w + 48 / g_qeglobals.d_xz.scale, 
				   g_qeglobals.d_xz.origin[nDim2] + h - 48 / g_qeglobals.d_xz.scale);
		glVertex2f(g_qeglobals.d_xz.origin[nDim1] - w + 48 / g_qeglobals.d_xz.scale, 
				   g_qeglobals.d_xz.origin[nDim2] + h - 32 / g_qeglobals.d_xz.scale);
		glEnd();
		glLineWidth(1);
	}
// <---sikk
...
void XZ_DrawSizeInfo (int nDim1, int nDim2, vec3_t vMinBounds, vec3_t vMaxBounds)
{
	...
	glVertex3f(vMinBounds[nDim1], 0.0f, vMinBounds[nDim2] - 6.0f  / g_qeglobals.d_xz.scale);
	glVertex3f(vMinBounds[nDim1], 0.0f, vMinBounds[nDim2] - 10.0f / g_qeglobals.d_xz.scale);

	glVertex3f(vMinBounds[nDim1], 0.0f, vMinBounds[nDim2] - 10.0f / g_qeglobals.d_xz.scale);
	glVertex3f(vMaxBounds[nDim1], 0.0f, vMinBounds[nDim2] - 10.0f / g_qeglobals.d_xz.scale);

	glVertex3f(vMaxBounds[nDim1], 0.0f, vMinBounds[nDim2] - 6.0f  / g_qeglobals.d_xz.scale);
	glVertex3f(vMaxBounds[nDim1], 0.0f, vMinBounds[nDim2] - 10.0f / g_qeglobals.d_xz.scale);

	glVertex3f(vMaxBounds[nDim1] + 6.0f  / g_qeglobals.d_xz.scale, 0.0f, vMinBounds[nDim2]);
	glVertex3f(vMaxBounds[nDim1] + 10.0f / g_qeglobals.d_xz.scale, 0.0f, vMinBounds[nDim2]);

	glVertex3f(vMaxBounds[nDim1] + 10.0f / g_qeglobals.d_xz.scale, 0.0f, vMinBounds[nDim2]);
	glVertex3f(vMaxBounds[nDim1] + 10.0f / g_qeglobals.d_xz.scale, 0.0f, vMaxBounds[nDim2]);

	glVertex3f(vMaxBounds[nDim1] + 6.0f  / g_qeglobals.d_xz.scale, 0.0f, vMaxBounds[nDim2]);
	glVertex3f(vMaxBounds[nDim1] + 10.0f / g_qeglobals.d_xz.scale, 0.0f, vMaxBounds[nDim2]);

	glEnd();

	glRasterPos3f(Betwixt(vMinBounds[nDim1], vMaxBounds[nDim1]), 0.0f, vMinBounds[nDim2] - 20.0  / g_qeglobals.d_xz.scale);
	sprintf(dimstr, g_pszDimStringsXZ[nDim1], vSize[nDim1]);
	glCallLists(strlen(dimstr), GL_UNSIGNED_BYTE, dimstr);

	glRasterPos3f(vMaxBounds[nDim1] + 16.0  / g_qeglobals.d_xz.scale, 0.0f, Betwixt(vMinBounds[nDim2], vMaxBounds[nDim2]));
	sprintf(dimstr, g_pszDimStringsXZ[nDim2], vSize[nDim2]);
	glCallLists(strlen(dimstr), GL_UNSIGNED_BYTE, dimstr);

	glRasterPos3f(vMinBounds[nDim1] + 4, 0.0f, vMaxBounds[nDim2] + 8 / g_qeglobals.d_xz.scale);
	sprintf(dimstr, g_pszOrgStringsXZ[1], vMinBounds[nDim1], vMaxBounds[nDim2]);
	glCallLists(strlen(dimstr), GL_UNSIGNED_BYTE, dimstr);
}
...
void XZ_Draw ()
{
	...
	if (g_qeglobals.d_xz.timing)
		start = Sys_DoubleTime();

	// clear
	g_qeglobals.d_xz.d_dirty = false;

	glViewport(0, 0, g_qeglobals.d_xz.width, g_qeglobals.d_xz.height);
	...
	mins[0] = g_qeglobals.d_xz.origin[nDim1] - w;
	maxs[0] = g_qeglobals.d_xz.origin[nDim1] + w;
	mins[1] = g_qeglobals.d_xz.origin[nDim2] - h;
	maxs[1] = g_qeglobals.d_xz.origin[nDim2] + h;
...
// <---sikk
	...
	if (!bFixedSize && drawn - nSaveDrawn > 0 && g_qeglobals.d_savedinfo.bShow_SizeInfo)
		XZ_DrawSizeInfo(nDim1, nDim2, vMinBounds, vMaxBounds);
	...
	XZ_DrawCameraIcon();
	XZ_DrawCoords();	// sikk - Draw Coords last so they are on top
    glFinish();
...
void XZ_Overlay ()
{
	...
	glOrtho(g_qeglobals.d_xz.origin[0] - w, 
		    g_qeglobals.d_xz.origin[0] + w, 
			g_qeglobals.d_xz.origin[1] - h, 
			g_qeglobals.d_xz.origin[1] + h, 
			-g_qeglobals.d_savedinfo.nMapSize, 
			g_qeglobals.d_savedinfo.nMapSize);// -8192, 8192); 	// sikk - Map Size
