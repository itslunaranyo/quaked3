...
//  File name: mru.c
//
//  Description:	Routines for MRU support
//
//  Development Team:	Gilles Vollant (100144.2636@compuserve.com) 
//
//*************************************************************
...
//  CreateMruMenu()
//
//  Purpose:	Allocate and Initialize an MRU and return a pointer on it
//
//  Parameters:
...
//
//  Parameters:
//      LPMRUMENU lpMruMenu	- pointer on MRUMENU, allocated by
//							  CreateMruMenu() or CreateMruMenuDefault()
//
//  Return: void
//
//  History:    Date       Author       Comment
...
//  SetNbLruShow()
//
//  Purpose:	Change the maximum number of item displayed on menu
//
//  Parameters:
...
//
//  Comments:	used when load .INI or reg database
//
//  History:    Date       Author       Comment
//              09/24/94   G. Vollant   Created
//
...
{                                      
	if (wItem >= NBMRUMENU) 
		return FALSE;
	_fstrncpy((lpMruMenu->lpMRU) + ((lpMruMenu->wMaxSizeLruItem) * (UINT)wItem),
...
//      FALSE - Function don't run successfully
//
//  Comments:	Used for saving in .INI or reg database, or when 
//				user select an MRU in File menu
//
//  History:    Date       Author       Comment
//              09/24/94   G. Vollant   Created
//
				  ...
				  BOOL fIDMBased, LPSTR lpItem, UINT uiSize)
{
	...
	if (wItem >= lpMruMenu->wNbItemFill) 
		return FALSE;
	_fstrncpy(lpItem,(lpMruMenu->lpMRU) + ((lpMruMenu->wMaxSizeLruItem) * (UINT)(wItem)), uiSize);
	*(lpItem + uiSize - 1) = '\0';
	return TRUE;
}
...
//  AddNewItem()
//
//  Purpose:	Add an item at the begin of the list
//
//  Parameters:
//      LPMRUMENU lpMruMenu	- pointer to MRUMENU
...
//      FALSE - Function don't run successfully
//
//  Comments:	Used when used open a file (using File Open common
...
//      FALSE - Function don't run successfully
//
//  Comments:	Used when used open a file, using MRU, and when an error
...
//  PlaceMenuMRUItem()
//
//  Purpose:	Add MRU at the end of a menu
//
//  Parameters:
//      LPMRUMENU lpMruMenu	- pointer on MRUMENU
...
//  Return: void
//
//  Comments:	Used MRU is modified, for refresh the File menu
//
//  History:    Date       Author       Comment
//              09/24/94   G. Vollant   Created
//
...
//  SaveMruInIni()
//
//  Purpose:	Save MRU in a private .INI
//
//  Parameters:
//      LPMRUMENU lpMruMenu	- pointer on MRUMENU
...
//
//
//  Comments:	See WritePrivateProfileString API for more info on lpszSection and lpszFile
//
//  History:    Date       Author       Comment
//              09/24/94   G. Vollant   Created
//
...
//  LoadMruInIni()
//
//  Purpose:	Load MRU from a private .INI
//
//  Parameters:
//      LPMRUMENU lpMruMenu	- pointer on MRUMENU
//      LPSTR lpszSection	- Points to a null-terminated string containing
...
//      FALSE - Function don't run successfully
//
//  Comments:	See GetPrivateProfileString API for more info on lpszSection and lpszFile
//
//  History:    Date       Author       Comment
//              09/24/94   G. Vollant   Created
//
...
//  SaveMruInReg()
//
//  Purpose:	Save MRU in the registry
//
//  Parameters:
//      LPMRUMENU lpMruMenu	- pointer on MRUMENU
...
//      FALSE - Function don't run successfully
//
//  Comments:	Win32 function designed for Windows NT and Windows 95
...
//  LoadMruInReg()
//
//  Purpose:	Load MRU from the registry
//
//  Parameters:
//      LPMRUMENU lpMruMenu	- pointer on MRUMENU
//      LPSTR lpszKey		- Points to a null-terminated string 
...
//  GetWin32Kind()
//
//  Purpose:	Get the Win32 platform
//
//  Parameters:
//
//  Return: (WIN32KIND)
